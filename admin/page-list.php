...
class Query_Wrangler_List_Table extends WP_List_Table
{
        ...
        switch($column_name){
            ...
            case 'details':
                ...
                if($item['type'] != 'override') {
                  $details.='Shortcode: [query id='.$item['ID'].']';
                }
                if($item['type'] == 'override'){
                  global $wpdb;
                  $terms = $wpdb->get_results("SELECT term_id FROM ".$wpdb->prefix."query_override_terms WHERE query_id = ".$item['ID']);
                  // build a list of overridden terms
                  if(is_array($terms)){
                    $overrides = array();
                    // loop through terms and provide links
                    foreach($terms as $term){
                      if($t = qw_get_term($term->term_id)){
                        $overrides[] = "<a href='".$t->link."' target='_blank'>".$t->name."</a>";
                      } else {
                        $overrides[] = "term: ".$term->term_id." not found";
                      }
                    }
                  }
                  $details.= 'Overriding: '.implode(' | ', $overrides);
                }
                return $details;
            default:
    ...
    function column_name($item){
        ...
        $actions = array(
            ...
            'export'    => sprintf('<a href="?page=%s&export=%s">Export</a>',$_REQUEST['page'], $item['ID']),
            'delete'    => sprintf('<a class="qw-delete-query" href="?page=%s&noheader=true&action=delete&edit=%s">Delete</a>',$_REQUEST['page'], $item['ID']),
        );
        // pages
        if($item['type'] == 'page'){
        ...
        return sprintf('%1$s <span style="color:silver">(ID: %2$s)</span> %3$s',
            /*$1%s*/ '<a href="?page=query-wrangler&edit='.$item['ID'].'">'.$item['name'].'</a>',
            /*$2%s*/ $item['ID'],
    ...
    function prepare_items() {
        ...
        $sql = "SELECT id as ID, type, name, path
...
function qw_list_queries_form(){
    ...
    if(!isset($_REQUEST['noheader'])){
      ...
      <div class="wrap">
          ...
          <h2>Query Wrangler  <a class="add-new-h2" href="admin.php?page=qw-create">Add New</a></h2>
